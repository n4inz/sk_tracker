<template>
    ...
    <div  ref="homeContent" class="sm:p-4 sm:ml-64">
        <div class="w-3/4 mx-auto flex flex-col p-4 border-2 border-blue-500 border-dashed rounded-lg dark:border-gray-700">
            <div class="mx-auto">
                <div class="sm:flex items-center space-x-8 max-w-sm p-6 bg-white  rounded-lg dark:bg-gray-800 dark:border-gray-700">
                    <div class="w-24  mx-auto mb-2">
                        <div :class="data.color + ' w-16 h-16 tex text-base sm:w-24 sm:h-24 sm:text-lg flex items-center justify-center text-white rounded-full'">
                    ...
                    <div>
                        <a href="#">
                            <h5 class="mb-2 text-sm sm:text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">{{ data.name }}</h5>
                        </a>
                        <p class=" font-normal  text-sm sm:text-base text-gray-500 dark:text-gray-400">@{{ data.username }}</p>
                        <span class=" font-normal text-sm sm:text-base text-gray-500 dark:text-gray-400">{{ data.email }}</span>
                    </div>
                </div>
            </div>

            <div v-if="jawaban !== null" class="grid sm:grid-cols-2 grid-cols-1 gap-4">
            ...
            <div v-else class="flex flex-col items-center justify-center h-48">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-24 h-24 sm:w-32 sm:h-32 text-gray-500 opacity-50">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25zM6.75 12h.008v.008H6.75V12zm0 3h.008v.008H6.75V15zm0 3h.008v.008H6.75V18z" />
                </svg>

                <h1 class="text-gray-500 text-base sm:text-1xl font-semibold text-center">User belum menjawab pertanyaan</h1>
            </div>
